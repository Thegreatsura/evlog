...
export async function exchangeTurboToken(oidcToken: string, team: string): Promise<string> {
  ...
  if (!response.ok) {
    throw eviErrors.TURBO_TOKEN_EXCHANGE_FAILED({
      message: `Turborepo token exchange failed (${response.status})`,
      internal: { status: response.status, body: await response.text() },
    })
  }
  const payload = await response.json() as { access_token?: unknown }
  const token = payload?.access_token
  if (typeof token !== 'string' || token.length === 0) {
    throw eviErrors.TURBO_TOKEN_MISSING()
  }
  return token
}
...
export function turboConfigCommand(token: string, teamId: string, teamSlug: string): string {
  ...
  for (const [name, value] of [['token', token], ['teamId', teamId], ['teamSlug', teamSlug]] as const) {
    if (!/^[\w.-]+$/.test(value)) throw eviErrors.TURBO_CONFIG_UNSAFE({ name })
  }
